...
const Item = ({ feature, isActive, onClick, index }) => {
  ...
  return (
    <div 
      className={`border-b border-white/10 relative transition-all duration-300 group ${isActive ? 'ios-glass-light' : 'hover:bg-white/5'}`}
    >
      <button
      ...
      >
        ...
        <span className={`transform transition-transform duration-300 ${isActive ? 'rotate-45' : 'rotate-0'} text-white/80`}>
      ...
      >
        <div className="pb-8 pt-2 px-6 pl-16 text-white/80 leading-relaxed max-w-3xl">
          {description}
        </div>
      </div>
    </div>
...
const FeaturesAccordion = () => {
  ...
  return (
    <section
      className="py-24 md:py-32 ios-bg-dark text-white relative overflow-hidden"
      id="features"
    >
      {/* Effets de lumière flottants */}
      <div className="absolute -top-40 -left-40 w-96 h-96 rounded-full bg-purple-500/20 blur-3xl animate-float"></div>
      <div className="absolute -bottom-40 -right-40 w-80 h-80 rounded-full bg-indigo-500/15 blur-3xl animate-float" style={{animationDelay: '2s'}}></div>
      <div className="absolute top-1/2 right-1/4 w-72 h-72 rounded-full bg-purple-400/10 blur-3xl animate-float" style={{animationDelay: '4s'}}></div>
      ...
      <div className="ios-container px-6 md:px-10 relative z-10">
        <div className="max-w-3xl mb-16 md:mb-24 ios-fade-in">
          ...
          <h2 className="ios-title leading-tight mb-8">
            ...
            <span className="bg-gradient-to-r from-purple-400 to-indigo-400 bg-clip-text text-transparent"> accessible à tous</span>
          </h2>
                ...
                <div className="relative w-full h-full flex items-center justify-center">
                  ...
                  <div className="w-full h-full flex items-center justify-center relative z-10">
                    <Image
                      ...
                      width={150}
                      height={150}
                      className="h-24 w-24 object-contain opacity-90 filter brightness-110"
                    />
                  </div>
                </div>
              </div>
