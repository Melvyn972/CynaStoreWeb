...
const Header = () => {
  ...
  // Effet pour marquer la session comme chargée
  useEffect(() => {
    if (status !== "loading") {
      ...
      <nav className="ios-container px-6 md:px-10 mx-auto py-4 z-nav">
        <div className="flex items-center justify-between z-nav">
          ...
          >
            ...
            <span className="font-bold text-xl text-gray-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-300 transition-colors">
              {config.appName}
            </span>
          </Link>
          ...
          <div className="hidden lg:flex lg:items-center lg:gap-8 relative z-nav">
            {links.map((link) => (
              <Link
                href={link.href}
                key={link.href}
                className="flex items-center gap-2 px-4 py-2 rounded-xl text-gray-700 dark:text-white/80 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/10 transition-all duration-300 group relative z-nav"
                title={link.label}
              >
                <span className="text-purple-600 dark:text-purple-400 group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors relative z-nav">
                  {link.icon}
                </span>
          ...
          <div className="hidden lg:flex lg:items-center lg:gap-4 relative z-nav">
            <NoSSR fallback={<div className="w-8 h-8 relative z-nav"></div>}>
            ...
            <NoSSR 
              fallback={
                <div className="ios-button-primary flex items-center gap-2 relative z-nav opacity-50">
                  <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                  Chargement...
                </div>
              }
            >
              {!sessionLoaded || status === "loading" ? (
                <div className="ios-button-primary flex items-center gap-2 relative z-nav opacity-50">
                  <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                  Chargement...
                </div>
              ) : (sessionLoaded && status === "authenticated" && session?.user) ? (
                <Link
                  href="/dashboard"
        ...
        <div className="fixed inset-y-0 right-0 w-full max-w-sm ios-glass border-l border-purple-500/20 transform transition-transform duration-300">
          <div className="flex flex-col h-full p-6">
            ...
            <div className="flex items-center justify-between mb-12">
              ...
              >
                <Image
                  ...
                  quality={90}
                />
                <span className="font-bold text-xl text-gray-900 dark:text-white">{config.appName}</span>
              </Link>
            ...
            <nav className="flex-1 space-y-4">
              {links.map((link) => (
                <Link
                  href={link.href}
                  key={link.href}
                  className="flex items-center gap-4 p-4 rounded-2xl text-gray-800 dark:text-white/90 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/10 transition-all duration-300 group"
                  onClick={() => setIsOpen(false)}
                >
                  <span className="text-purple-600 dark:text-purple-400 group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors">
                    {link.icon}
                  </span>
            ...
            <div className="pt-6 border-t border-gray-300 dark:border-white/10">
              {!sessionLoaded || status === "loading" ? (
                <div className="w-full ios-button-primary justify-center opacity-50">
                ...
                <Link
                  href="/dashboard"
                  className="flex items-center gap-4 p-4 rounded-2xl ios-glass-light hover:bg-gray-100 dark:hover:bg-white/20 transition-all duration-300"
                  onClick={() => setIsOpen(false)}
                >
                  {session.user?.image ? (
                    <Image
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600 dark:text-white/60">Voir le tableau de bord</div>
                  </div>
                </Link>
              ) : (
                <Link
